...
import { VINYL_OPTIONS, VINYL_BASE_PRICING } from '../constants';
...
const calculateVinylPrice = (config: { size: string, weight: string, color: string, innerSleeve: string, outerSleeve: string, units: number }): number => {
    ...
    const outerSleeveMod = VINYL_OPTIONS.outerSleeves[size as keyof typeof VINYL_OPTIONS.outerSleeves]?.find(s => s.id === outerSleeve)?.priceModifier || 0;
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select id="vinyl-outer" value={outerSleeve} onChange={e => setOuterSleeve(e.target.value)} className="w-full p-2 bg-gray-100 text-black focus:outline-none focus:ring-2 focus:ring-black">
                                        {VINYL_OPTIONS.outerSleeves[size as keyof typeof VINYL_OPTIONS.outerSleeves].map(opt => <option key={opt.id} value={opt.id}>{opt.label} (+{opt.priceModifier.toFixed(2)}€)</option>)}
                                    </select>
                                </div>
                            </div>
                            {/* Units */}
